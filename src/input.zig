...
pub const Input = struct {
    ...
    pub fn readChar(self: *Self, stdout: *writer.Writer) !bool {
        ...
        switch (char) {
            ...
            127 => {
                ...
                if (self.cursor_pos == self.buffer.items.len) {
                    ...
                    // ANSI backspace
                    try stdout.writer.print("\x1b[D\x1b[K", .{});
                } else {
                    // Remove character from buffer
                    ...
                    try stdout.writer.print("{s}", .{tail});
                    for (0..tail.len) |_| {
                        try stdout.writer.print("\x1b[D", .{});
                    }
                }
            ...
            '\n' => {
                ...
                if (self.cursor_pos == self.buffer.items.len) {
                    ...
                    const tail = self.buffer.items[self.cursor_pos - 1 ..];
                    try stdout.writer.print("\x1b[K{s}", .{tail});
                    for (0..tail.len - 1) |_| {
                        try stdout.writer.print("\x1b[D", .{});
                    }
                }
    ...
    pub fn readLine(self: *Self, stdout: *writer.Writer) ![]const u8 {
        self.clear();
